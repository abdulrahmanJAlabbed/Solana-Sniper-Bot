...
exports.getRugCheckConfirmed = getRugCheckConfirmed;
const axios_1 = __importDefault(require("axios"));
const dotenv_1 = __importDefault(require("dotenv"));
const config_1 = require("../../config");
const db_1 = require("../../tracker/db");
...
async function getRugCheckConfirmed(tokenMint) {
    try {
        const rugResponse = await axios_1.default.get(`https://api.rugcheck.xyz/v1/tokens/${tokenMint}/report`, {
            timeout: config_1.config.axios.get_timeout,
        });
        if (!rugResponse.data)
            return false;
        // For debugging purposes, log the full response data
        if (config_1.config.checks.verbose_logs) {
        ...
        if (config_1.config.checks.settings.exclude_lp_from_topholders) {
            const markets = tokenReport.markets;
            if (markets) {
                // Safely extract liquidity addresses from markets
                const liquidityAddresses = (markets ?? [])
        ...
        // Get config settings
        const rugCheckSettings = config_1.config.checks.settings;
        ...
        if (rugCheckSettings.block_returning_token_names || rugCheckSettings.block_returning_token_creators) {
            try {
                ...
                const duplicate = await (0, db_1.selectTokenByNameAndCreator)(tokenName, tokenCreator);
        ...
        try {
            await (0, db_1.insertNewToken)(newToken);
        }
        catch (err) {
            if (rugCheckSettings.block_returning_token_names || rugCheckSettings.block_returning_token_creators) {
